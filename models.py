...
def predict_df(unscaled_predictions, original_df):
    ...
    # Determine the valid range for iterating through sales_dates
    valid_range = min(len(unscaled_predictions), len(sales_dates) - 1)
    for index in range(0, valid_range):
        if isinstance(unscaled_predictions, pd.core.frame.DataFrame):
            unscaled_predictions = unscaled_predictions.values
        result_dict = {}
...
def regressive_model(train_data, test_data, model, model_name):
    ...
    original_df = load_data('train_data\\monthly_data.csv')
    ...
    # pickle.dump(scaler_object, open(scaler_filename, 'wb'))
...
def lstm_model(train_data, test_data):
    ...
    # Save the unscaled predictions
    # predictions_filename = 'lstm_predictions.pkl'
    # pickle.dump(unscaled_df, open(predictions_filename, 'wb'))
...
def sarimax_model(data):
    ...
    # pickle.dump(sar, open(model_filename, 'wb'))

    # Save the unscaled predictions
    # predictions_filename = 'arima_predictions.pkl'
    # pickle.dump(unscaled_df, open(predictions_filename, 'wb'))
...
def main():
    ...
    """
    # Regression models
    model_df = load_data('train_data\\model_df.csv')
    ...
    # Arima
    ts_data = load_data('train_data\\arima_df.csv').set_index('date')
